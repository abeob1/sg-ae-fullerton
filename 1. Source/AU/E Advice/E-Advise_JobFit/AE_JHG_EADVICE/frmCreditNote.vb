...
Public Class frmCreditNote
    ...
    Private Function ExportReports(ByRef sErrdesc As String) As Long
        ...
        Try
            ...
            Select Case DocType.Text.ToUpper().Trim()
                Case "INJURY MANAGEMENT"
                    sRptFileName = p_oCompDef.sReportsPath & "\JHG_CreditNote_Injury Management.rpt"
                    sSQL += " AND T2.""IndName"" = 'Injury'"
                Case "PREEMPLOYMENT"
                    sRptFileName = p_oCompDef.sReportsPath & "\JHG_CreditNote_PreEmp.rpt"
            ...
            For imjs As Integer = 0 To gvtaxinvoice.Rows.Count - 1
                If Convert.ToBoolean(gvtaxinvoice.Rows(imjs).Cells(0).Value) = True Then
                    ...
                    lblMsg.Text = "Processing the Credit Note " & sInvoice
                    Me.Refresh()

                    sSQL = "SELECT T1.""E_Mail"",T0.""DocEntry"",T0.""CardCode"",T0.""DocNum"",T0.""CardName"", T0.""DocDate"" " & _
           " FROM " & """" & Me.SCompany.Text & """" & ".ORIN T0  LEFT JOIN " & """" & Me.SCompany.Text & """" & ".OCRD T1" & _
                    ...
                    If oDS.Tables(0).Rows.Count > 0 Then
                        ...
                        For Each row As DataRow In oDS.Tables(0).Rows
                            If row.Item("E_Mail").ToString <> String.Empty Then
                                ...
                                sTargetFileName = "CREDITNOTE_" & row.Item("DocNum").ToString & "_" & row.Item("CardCode").ToString & "_" & dDateCon.ToString("ddMMyyyy") & ".pdf"
                                ...
                                sBody = sBody & "Credit Note     : " & row.Item("DocNum").ToString.Trim & ".<br /><br/><br/>"

                                Select Case DocType.Text.ToUpper.Trim()
                                    Case "INJURY MANAGEMENT"
                                        ' sSubject = "Invoices / Statements (Injury Management)"
                                        sSubject = "Credit note from Jobfit Health Group Pty Ltd (" & SCompany.Text.ToString().Trim() & ")"
                                        sBody = sBody & " Please find attached your credit note/statement.  If you have any queries please contact us on 8177 8600 or email injurymanagement.accounts@jobfit.com.au <br /><br />"
                                    Case Else
                                        ''   sSubject = "Invoices / Statements (Pre-employment)"
                                        sSubject = "Credit note from Jobfit Health Group Pty Ltd (" & SCompany.Text.ToString().Trim() & ")"
                                        sBody = sBody & " Please find attached your credit note/statement.  If you have any queries please contact us on 8177 8600 or email accounts.receivable@jobfit.com.au <br /><br />"
                                End Select
    ...
    Private Sub Button1_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Dim sBPCode As String = String.Empty
        Dim sBpcodeTo As String = String.Empty
        Dim sFuncName As String = "Credit Note Show()"
        Dim sSQL As String = String.Empty
        Dim oDS As New DataSet
        ...
        If Not Me.DocType.Text = "SOA" Then
            If Me.txtDocNumFrom.Text = String.Empty And Me.txtDocNumTo.Text = String.Empty And (Me.txtBpCode.Text = String.Empty And Me.CheckBox1.Checked = False) _
                And Me.DateTimePicker4.Checked = False And Me.DateTimePicker5.Checked = False Then
                MsgBox("Enter either DocNum range OR DocDate range OR BP Group", MsgBoxStyle.Information, "E-Advice")
                Exit Sub
            End If
        ...
        If Me.CheckBox1.Checked = True Then
            sBPCode = "%"
        ElseIf String.IsNullOrEmpty(Me.txtBpCode.Text) Then
            sBPCode = "%"
        Else
        ...
        If (Me.txtDocNumFrom.Text <> String.Empty And Me.txtDocNumTo.Text <> String.Empty) And _
                      (Me.DateTimePicker4.Checked = False And Me.DateTimePicker5.Checked = False) Then
        ...
        ElseIf (Me.txtDocNumFrom.Text <> String.Empty And Me.txtDocNumTo.Text <> String.Empty) And _
           (Me.DateTimePicker4.Checked = True And Me.DateTimePicker5.Checked = True) Then
  ...
  (Me.DateTimePicker4.Checked = False And Me.DateTimePicker5.Checked = False) Then
        ...
        If sBPCode <> "%" Then
            sSQL += " AND T1.""GroupCode"" like '" & sBPCode & "'"
        End If


        sSQL += "GROUP BY T1.""E_Mail"",T0.""DocEntry"",T0.""CardCode"",T0.""DocNum"",T0.""CardName"", T0.""DocDate"", T1.""U_AI_EMAILSOA"""

        If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Attempting to Payment Advise Portion :: ", sFuncName)

        If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Execute SQL" & sSQL, sFuncName)
        oDS = ExecuteSQLQuery(sSQL)
        If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling Load_CreditNote()" & sSQL, sFuncName)
        If Load_CreditNote(oDS, sErrdesc) <> RTN_SUCCESS Then Throw New ArgumentException(sErrdesc)
    ...
    Private Function Load_CreditNote(ByVal oDS As DataSet, ByRef sErrDesc As String) As Long

        ' **********************************************************************************
        '   Function    :   Load_CreditNote()
        ...
        Try
            ...
            Load_CreditNote = RTN_SUCCESS

        Catch ex As Exception
            WriteToLogFile(ex.Message, sFuncName)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            Load_CreditNote = RTN_ERROR
        End Try
    End Function
    ...
    Private Sub UpdateFlag(ByVal DocEntry As Integer, ByVal sCompany As String)
        ...
        Try
            ...
            sSQL = "UPDATE " & sCompany & ".ORIN SET ""U_AI_EINVOICE""='Yes' Where ""DocEntry""=" & DocEntry
