...
Public Class frmSOA
                ...
                Case "SOA"
                    ...
                    For imjs As Integer = 0 To Dgv_BPList.Rows.Count - 1
                        ...
                        If Convert.ToBoolean(Dgv_BPList.Rows(imjs).Cells(0).Value) = True Then
                            ...
                            sCardcode = Convert.ToString(Dgv_BPList.Rows(imjs).Cells(1).Value).Replace("'", "%&")
                            ...
                            If sEmail <> String.Empty Then
                                ...
                                If oDS1.Tables(0).Rows.Count > 0 Then
                                    ...
                                    End If
                                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Successfully  generated PDF ::" & sTargetFileName, sFuncName)
                                    ...
                                    sBody = sBody & "Statement of Account as on " & Me.DateTimePicker1.Value.ToString("dd-MMM-yyyy") & ".<br /><br/><br/>"
                                    ...
                                    sSubject = "Statement of Accounts from Jobfit Health Group Pty Ltd (" & Me.SCompany.Text & ")"
    ...
    Private Function Load_BPGroupList(ByRef sErrDesc As String, ByVal sCompanyName As String, ByVal sGroupCode As String) As Long
        ...
        Try
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Checking BP Balance ..", sFuncName)
            lblMsg.Text = " Please Wait, It`s Loading the Information  ::"
            Me.Refresh()
            Dim dDateCon As Date = DateTimePicker1.Text
            dDateCon.ToString("yyyyMMdd")
            sSQL = "CALL " & """" & Me.SCompany.Text & """" & ".""AE_SP003_SOA_InitialCheck"" ('" & dDateCon.ToString("yyyyMMdd") & "')"
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Execute SQL" & sSQL, sFuncName)
            ...
            sSQL = "SELECT T0.""CardCode"", T0.""CardName"",T0.""E_Mail"", T0.""U_AI_EMAILSOA"" from " & """" & Me.SCompany.Text & """" & ".OCRD T0 " & _
                " inner join " & """" & Me.SCompany.Text & """" & ".AE_TMP_SOAFN T1 on T1.""CardCode"" = T0.""CardCode""  " & _
                          " WHERE T0.""frozenFor"" ='N' AND T0.""GroupCode"" like '" & sGroupCode & "' AND T0.""CardType"" = 'C' AND T0.""U_AI_EMAILSOA"" = 'YES'  " & _
                          " AND T0.""CardCode"" NOT IN (select TT.""U_AI_CARDCODE"" from " & """" & Me.SCompany.Text & """" & ".""@SOAEMAILLOG"" TT where TT.""U_AI_SOADATE"" = '" & dDateCon.ToString("yyyyMMdd") & "')" & _
                          "group by T0.""CardCode"", T0.""CardName"",T0.""E_Mail"", T0.""U_AI_EMAILSOA"" Order by  T0.""CardCode"""
            ...
            lblMsg.Text = "Loading Completed Successfully ::"
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
