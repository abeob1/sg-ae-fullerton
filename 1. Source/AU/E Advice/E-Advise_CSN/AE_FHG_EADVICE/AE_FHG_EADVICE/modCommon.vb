...
Module modCommon
    ...
    Public strServer As String = ConfigurationManager.AppSettings("CSNServer").ToString()
    Public strUserName As String = ConfigurationManager.AppSettings("CSNUserName").ToString()
    Public strPassword As String = ConfigurationManager.AppSettings("CSNPassword").ToString()
    Public strFromMail As String = ConfigurationManager.AppSettings("CSNFromEmail").ToString()
    Public strEWSUrl As String = ConfigurationManager.AppSettings("EWSUrl").ToString()
    ...
    Public Function SendEmailNotification_B(ByVal sfileName As String, _
        ...
        Try
            ...
            oMail.From = New System.Net.Mail.MailAddress(sFromEmail)
            ...
            oMail.Attachments.Add(New System.Net.Mail.Attachment(sfileName))
            ...
            SendEmailNotification_B = RTN_SUCCESS
        Catch ex As Exception
            ...
            SendEmailNotification_B = RTN_ERROR
        Finally
    ...
    Public Function SendEmailNotification(ByVal sfileName As String, _
        ...
        Try
            ...
            frmSOA.WriteToStatusScreen(False, "Sending Email to :: " & sSenderEmail)
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Sending Email to :: " & sSenderEmail, sFuncName)
    ...
    Public Function SendEmailNotificationForCSN(ByVal sfileName As String, _
                                          ByVal sFromEmail As String, _
                                          ByVal sSenderEmail As String, _
                                          ByVal sBody As String, _
                                          ByVal sSubject As String, _
        ...
        Try
            sFuncName = "SendEmailNotificationForCSN()"
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Starting Function...", sFuncName)
            Console.WriteLine("Starting Function... " & sFuncName)
            ...
            service.Credentials = New NetworkCredential(strUserName.ToString(), strPassword.ToString())
            service.Url = New Uri(strEWSUrl.ToString())
            Dim message As New EmailMessage(service)
            'message.From = sFromEmail
            message.Subject = sSubject
            message.Body = sBody
            message.Attachments.AddFileAttachment(sfileName)
            Dim sSendTo As String() = sSenderEmail.Split(";")
            For Each sEmailAddress In sSendTo
                message.ToRecipients.Add(sEmailAddress)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Adding Email Address" & ":  " & sEmailAddress, sFuncName)
            Next
            ...
            Console.WriteLine("Completed with SUCCESS " & sFuncName)
            frmSOA.WriteToStatusScreen(False, "Email Has been sent successfully :: " & sSenderEmail)
            SendEmailNotificationForCSN = RTN_SUCCESS

            'Dim oMail As New EASendMail.SmtpMail("TryIt")
            ' '' Dim oMail As New EASendMail.SmtpMail("6RXE6-R9U2K-HFWZ5-K4186")
            ...
            'oMail.AddAttachment(sfileName)
            'oMail.Subject = sSubject
            ' ''  oMail.TextBody = sBody
            'oMail.HtmlBody = sBody

            'Dim oServer As New SmtpServer(strServer.ToString())
            'oServer.User = strUserName.ToString()

            'oServer.Password = strPassword.ToString()
        ...
        Catch ex As Exception
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            Console.WriteLine("Completed with Error " & sFuncName)
            SendEmailNotificationForCSN = RTN_ERROR
        Finally
            File.Delete(sfileName)
        End Try
                ...
                For Each row As DataRow In oDSCardCode.Tables(0).Rows
                    ...
                    If oDS.Tables(0).Rows.Count > 0 Then
                        If Not String.IsNullOrEmpty(oDS.Tables(0).Rows(0).Item("E_Mail").ToString) Then
                            ...
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("ExportToPDF" & sSQL, sFuncName)
                            frmSOA.WriteToStatusScreen(False, "Attempting Function ExportToPDF  :: ")
                            If ExportToPDF_SOA(CardCodeFrom, CardCodeTo, _Date, sTargetFileName, sRptFileName, frmSOA.SCompany.Text, sErrDesc) <> RTN_SUCCESS Then
                                Throw New ArgumentException(sErrDesc)
                            End If
                            frmSOA.WriteToStatusScreen(False, "Successfully  generated PDF :: " & sTargetFileName)
                            'If SendEmailNotification(sTargetFileName, oDS.Tables(0).Rows(0).Item("E_Mail").ToString, "", sErrDesc) <> RTN_SUCCESS Then Throw New ArgumentException(sErrDesc)
        ...
        Try
            ...
            If oDS.Tables(0).Rows.Count > 0 Then
                If Not String.IsNullOrEmpty(oDS.Tables(0).Rows(0).Item("E_Mail").ToString) Then
                    ...
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("ExportToPDF" & sSQL, sFuncName)
                    frmSOA.WriteToStatusScreen(False, "Attempting Function ExportToPDF  :: ")
                    If ExportToPDF_SOA(CardCode, CardCode, _Date, sTargetFileName, sRptFileName, frmSOA.SCompany.Text, sErrDesc) <> RTN_SUCCESS Then
                        Throw New ArgumentException(sErrDesc)
                    End If
                    frmSOA.WriteToStatusScreen(False, "Successfully  generated PDF :: " & sTargetFileName)
                    ' If SendEmailNotification(sTargetFileName, oDS.Tables(0).Rows(0).Item("E_Mail").ToString, "", sErrDesc) <> RTN_SUCCESS Then Throw New ArgumentException(sErrDesc)
    ...
    Public Function ExecuteSQLQuery_DataTable(ByVal sQuery As String, ByVal sCompanyDB As String, _
                                              ByRef sErrDesc As String) As DataTable

        '**************************************************************
        ' Function      : ExecuteSQLQuery_DataTable
        ' Purpose       : Execute SQL
        ' Parameters    : ByVal sSQL - string command Text
        ' Author        : Sri
        ' Date          : 
        ...
        Catch ex As Exception
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed wiith ERROR.", sFuncName)
            Throw New Exception(ex.Message)
        Finally
            oCon.Dispose()
        End Try
