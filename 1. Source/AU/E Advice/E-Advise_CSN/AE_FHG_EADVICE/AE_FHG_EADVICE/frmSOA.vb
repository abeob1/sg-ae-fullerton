...
Public Class frmSOA
    ...
    Private Function ExportReports(ByRef sErrdesc As String) As Long
        ...
        Dim sCompName As String = String.Empty
        sCompName = (ConfigurationManager.AppSettings("CompanyName").ToString())
        Dim sAddressLine1 As String = String.Empty
        sAddressLine1 = ConfigurationManager.AppSettings("AddressLine1").ToString()
        Dim sAddressLine2 As String = String.Empty
        sAddressLine2 = ConfigurationManager.AppSettings("AddressLine2").ToString()
                        ...
                        If Convert.ToBoolean(Dgv_BPList.Rows(imjs).Cells(0).Value) = True Then
                            ...
                            If sEmail <> String.Empty Then
                                ...
                                If oDS1.Tables(0).Rows.Count > 0 Then
                                    ...
                                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling Function ExportToPDF_SOA()", sFuncName)
                                    ...
                                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Created the PDF", sFuncName)
                                    ...
                                    sBody = sBody & sCompName & " <br/> "
                                    sBody = sBody & sAddressLine1 & " <br/> "
                                    sBody = sBody & sAddressLine2 & " <br/> <br/>"
                                    sBody = sBody & " <B>T</B> +61 2 8256 1704 | <B>F</B> +61 2 82561785 | <B>W</B> fullertonhealthcare.com.au. <br/> <br/><br/>"

                                    'sBody = sBody & " NOTICE - This  email and any attachments may contain confidential, privileged and/or copyright information " & _
                                    '    " that is only for use by the intended recipient.  If you received it in error, please immediately notify the sender, and destroy " & _
                                    '    " this email and any attachments, and do not in any way make use of its contents (which includes any copying, reading, reliance," & _
                                    '    " storage, transmission, dissemination or disclosure).  Unauthorised use of this email is not permitted. " & _
                                    '    " Fullerton Healthcare Australia Pty Ltd ACN 164 892 924.<br/><br /><br />"
                                    ...
                                    '' sFromEmail = "soa.pa@fullertonhealthcs.com.au"
                                    Me.Refresh()
                                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling Function SendEmailNotification()", sFuncName)
                                    ''If SendEmailNotificationForCSN(sTargetFileName, sFromEmail, "sahayar@abeo-electra.com", sBody, sSubject, sErrdesc) <> RTN_SUCCESS Then Throw New ArgumentException(sErrdesc)
                                    ''If SendEmailNotificationForCSN(sTargetFileName, sFromEmail, sEmail, sBody, sSubject, sErrdesc) <> RTN_SUCCESS Then Throw New ArgumentException(sErrdesc)
